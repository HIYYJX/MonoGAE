...
class DDNLoss(nn.Module):
    ...
    def __init__(self,
                 ...
                 fg_weight=1,
                 bg_weight=1,
                 downsample_factor=1): # 注意，我们的为fg_weight=1   monodetr为13
        """
    ...
    def bin_depths(self, depth_map, mode="LID", depth_min=1e-3, depth_max=200, num_bins=270, target=False):
        """
    ...
    def forward(self, depth_logits, gt_boxes2d, num_gt_per_img, gt_center_depth,depthmaps):
        """
        Gets depth_map loss
        Args:
        ...
        #print(depth_maps.shape)torch.Size([8, 32, 58])
        depthmaps = depthmaps.squeeze(-1)
        #print(depthmaps.shape)
        depthmaps = depthmaps.transpose(2,1)
        #rint(depthmaps.shape)

        #depth_target = self.bin_depths(depth_maps, target=True)
        depth_target = self.bin_depths(depthmaps, target=True)
