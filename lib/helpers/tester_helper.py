...
class Tester(object):
    ...
    def inference(self):
        ...
        model_infer_time = 0
        for batch_idx, (inputs, calibs, targets, info,random_flip_flag) in enumerate(self.dataloader):
            ...
            #trans_inv = targets['trans_inv'].to(self.device)
            trans_inv = targets['trans_inv'] 
            depthmaps = targets['depthmaps']

            start_time = time.time()
            outputs = self.model(inputs, calibs, targets, img_sizes,ori_denorms,random_flip_flag)
            denorm = outputs['pred_denorms']##8,32,58,4
            #denorm =  targets['ori_denorms'] #8 58,32,4
            #denorm =  targets['denorms'] 
            #denorm = denorm.permute(0,2,1,3)#8 58,32,4
            weighted_depth=  outputs['weighted_depth'] #   8,32,58
            end_time = time.time()
